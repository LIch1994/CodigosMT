...
SCALE_FACTOR = 4 # used to resize the captured frame for face detection for faster processing speed //utilizado para cambiar el tamano del fotograma capturado de deteccion de rostros para mayor velocidad de procesamiento
...
# dictionary mapping used to keep track of head rotation maps //mapeo  utiliza para realizar un seguimiento de los mapas de rotacion de la cabeza
rotation_maps = {
...
while ret:
    ...
    if key in [27, ord('Q'), ord('q')]: 
        break
    # resize the captured frame for face detection to increase processing speed // cambiar el tamano del fotograma capturado para la deteccion de cara a aumentar la velocidad de procesamiento
    ...
    if frame_skip_rate == 0:
        ...
        for rotation in current_rotation_map:
            ...
            # If frontal face detector failed, use profileface detector // Si la deteccion de caras frontal fallo, perfil de usuario cara de deteccion Otro xml
            ...
            if len(faces):
                for f in faces:
                    ...
                    x, y, w, h = [ v for v in f ] # scale the bounding box back to original frame size // escalar el cuadro delimitador de nuevo a tamano trama original
